...
Source-path lines the tasks do not emit keep `source_kind` at the top level: the event loop's `block source error` and `reconnect task ended unexpectedly`, and the two cursor-commit failures.
A query that wants every source line reads `(.source_kind // .span.source_kind)`.
