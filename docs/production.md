...
| `nexum_runtime_source_reconnects_total` | counter | `source_kind`, `chain_id`, `module` | Source reconnects. `source_kind="block"` is per chain; `source_kind="chain-log"` also carries `module`. Every log line on both source paths carries the same value in a `source_kind` field, so the value `NexumReconnectStorm` reports greps the logs. |
