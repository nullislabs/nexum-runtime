...
| `nexum_runtime_module_memory_bytes` | gauge | `module` | The module's largest linear memory as of its last dispatch, against `[component.resources] max_memory_bytes` clamped to the `[policy]` value, which is a per-memory ceiling. Read on the growth path itself, and wasm linear memory never shrinks, so the value is the size the module reached rather than a sample of it. It includes the memory the instance starts with, so a module that never grows still reports its initial size. A restart rebuilds the store, so the series drops back to the reloaded module's first growth. |
