...
`nexum:host/chain` lives in `wit/nexum-host/chain.wit`:
...
`method` carries the namespace prefix, for example `eth_call`.
...
The enum is the structural ceiling for every host.
...
- `block_on` drives the returned futures.
  ...
  A `Pending` panics, which signals that an alloy layer awaiting a reactor or a timer has been introduced.
...
A component that only needs raw JSON calls `host.request(chain_id, method, params)` directly, or reaches for the `chain::eth_call_params` and `chain::parse_eth_call_result` helpers.
