...
///
/// Apply to an `impl` block whose associated functions are the trigger
/// handlers (`init`, `on_block`, `on_event`, `on_schedule`,
/// `on_extension`); each takes its trigger's wit-bindgen
...
///
/// `sol_events(EventType, ...)` fails the build unless the named Solidity
/// events' `SolEvent::SIGNATURE_HASH` values and the manifest's event trigger
...
fn expand(args: ModuleArgs, input: ItemImpl) -> Result<proc_macro2::TokenStream, MacroError> {
    ...
    // A typo'd handler (`on_blocks`, `on_events`, ...) would otherwise
    // compile as an ordinary helper while its trigger silently no-ops, so
    ...
    let ManifestFacts {
        ...
        .and_then(|dir| derive_manifest_facts(&dir, !args.sol_events.is_empty()))?;
    let parity = topic_parity_check(&args.sol_events, &event_topics);
    ...
    let event_arm = dispatch_arm(self_ty, &present, "on_event", "Event");
    let schedule_arm = dispatch_arm(self_ty, &present, "on_schedule", "Schedule");
    let extension_arm = dispatch_arm(self_ty, &present, "on_extension", "Extension");
...
enum MacroError {
    ...
    EmptySolEvents { span: proc_macro2::Span },
    #[error(transparent)]
    ...
    #[error(
        "#[nexum_sdk::module] found no recognized handlers on this impl; define at least one \
         of `init`, `on_block`, `on_event`, `on_schedule`, `on_extension`"
    )]
    ...
    #[error(
        "`sol_events(...)` names events, but {path} declares no event trigger with \
         an `event_signature`; add the trigger or drop the argument"
    )]
    SolEventsWithoutEventTrigger { path: String },
}
...
impl MacroError {
    ...
    fn into_syn_error(self) -> syn::Error {
        ...
        match self {
            ...
            | Self::EmptySolEvents { span } => syn::Error::new(span, message),
            ...
            | Self::WitResolution { .. }
            | Self::SolEventsWithoutEventTrigger { .. } => {
...
/// The macro's arguments: bare, or `sol_events(EventType, ...)`.
struct ModuleArgs {
    sol_events: Vec<syn::Path>,
}
...
fn parse_args_inner(
    ...
    if paths.is_empty() {
        return refuse(input, MacroError::EmptySolEvents { span: ident.span() });
    }
    Ok(Ok(ModuleArgs {
        sol_events: paths.into_iter().collect(),
    }))
}
...
struct ManifestFacts {
    ...
    /// Distinct event trigger `event_signature` topics, in declaration order.
...
fn derive_manifest_facts(
    ...
    let event_topics = if want_topics {
        nexum_world::manifest_event_topics(&text).map_err(|source| MacroError::Manifest {
            path: manifest_path.clone(),
            source: Box::new(source),
        })?
    } else {
    ...
    if want_topics && event_topics.is_empty() {
        return Err(MacroError::SolEventsWithoutEventTrigger {
            path: manifest_path,
        });
    }
    Ok(ManifestFacts {
...
/// Const assertions pinning set equality between the `sol_events(...)`
/// events' topic-0 hashes and the manifest's event topics. Const
...
fn topic_parity_check(events: &[syn::Path], topics: &[B256]) -> proc_macro2::TokenStream {
    ...
    let declared_checks = events.iter().enumerate().map(|(i, path)| {
        let msg = format!(
            "topic drift: `{}`'s topic-0 is not among the component.toml event trigger \
             event_signature values [{manifest_list}]",
            path_string(path),
        );
        quote! {
            ::core::assert!(
                ::nexum_sdk::sol_events::contains_topic(&DECLARED[#i], &MANIFEST),
                #msg,
            );
        }
    });
    let manifest_checks = topics.iter().enumerate().map(|(j, topic)| {
        let msg = format!(
            "topic drift: component.toml event trigger event_signature {topic} is not the topic-0 \
             of any of sol_events({declared_list})",
        );
        quote! {
            ::core::assert!(
                ::nexum_sdk::sol_events::contains_topic(&MANIFEST[#j], &DECLARED),
                #msg,
            );
        }
    });
...
/// One `on_trigger` arm; an absent handler dispatches as a no-op.
fn dispatch_arm(
    self_ty: &syn::Type,
    present: &[&str],
    handler: &str,
    ...
    if present.contains(&handler) {
        ...
        quote! { nexum::host::types::Trigger::#variant(payload) => <#self_ty>::#call(payload), }
    } else {
        quote! { nexum::host::types::Trigger::#variant(_) => ::core::result::Result::Ok(()), }
    }
}
...
mod tests {
    ...
    fn bare_attribute_names_no_events() {
        assert!(parse_args(quote! {}).unwrap().sol_events.is_empty());
    }

    #[test]
    fn sol_events_parses_paths_in_order() {
        let args = parse_args(quote! { sol_events(OrderPlacement, events::Refund) }).unwrap();
        let names: Vec<String> = args.sol_events.iter().map(path_string).collect();
        assert_eq!(names, ["OrderPlacement", "events::Refund"]);
    }

    #[test]
    fn empty_sol_events_is_rejected() {
        let err = parse_args(quote! { sol_events() }).err().unwrap();
        assert!(matches!(err, MacroError::EmptySolEvents { .. }), "{err:?}");
    }
    ...
    fn trailing_tokens_are_rejected() {
        let err = parse_args(quote! { sol_events(Foo), extra }).err().unwrap();
    ...
    fn malformed_args_pass_through_syn_diagnostics() {
        let err = parse_args(quote! { sol_events(1) }).err().unwrap();
    ...
    fn present_handler_arm_calls_the_impl() {
        let ty: syn::Type = syn::parse_quote!(Alerts);
        let arm = flat(dispatch_arm(&ty, &["on_block"], "on_block", "Block"));
        assert_eq!(
            arm,
            "nexum::host::types::Trigger::Block(payload)=><Alerts>::on_block(payload),",
        );
    }
    ...
    fn absent_handler_arm_is_a_no_op() {
        let ty: syn::Type = syn::parse_quote!(Alerts);
        let arm = flat(dispatch_arm(&ty, &["on_block"], "on_schedule", "Schedule"));
        assert_eq!(
            arm,
            "nexum::host::types::Trigger::Schedule(_)=>::core::result::Result::Ok(()),",
        );
    }

    /// Every handler dispatches on its own trigger variant: a payload can
    ...
    fn each_handler_binds_its_own_variant() {
        ...
        for (handler, variant) in pairs {
            ...
            assert!(
                arm.contains(&format!("Trigger::{variant}(payload)")),
                "{arm}"
            );
            assert!(
                arm.contains(&format!("<Alerts>::{handler}(payload)")),
                "{arm}"
            );
        }
    }
    ...
    fn defined_init_export_forwards_the_config() {
        ...
        assert!(emitted.contains("<Alerts>::init(config)"), "{emitted}");
    }
    ...
    fn absent_init_export_is_a_no_op() {
        ...
        assert!(!emitted.contains("Alerts>::init"), "{emitted}");
    }
    ...
    fn parity_check_pins_both_directions() {
        ...
        assert!(
            emitted.contains("is not the topic-0 of any of sol_events(OrderPlacement)"),
            "{emitted}",
        );
    ...
    /// A manifest field only `sol_events(...)` reads must not fail the build
    ...
    /// `sol_events(...)` needs a manifest event trigger; one with
    ...
    fn sol_events_without_event_trigger_is_refused() {
        ...
        let err = derive_manifest_facts(dir.path(), true).err().unwrap();
        assert!(
            matches!(err, MacroError::SolEventsWithoutEventTrigger { .. }),
            "{err:?}"
        );
    ...
    /// An event trigger with a valid `event_signature`, appended
    ...
    mod wording {
        ...
        fn non_ident_argument() {
            assert_eq!(
                ...
                "expected `sol_events(EventType, ...)` or no arguments",
            );
        }
        ...
        fn unknown_argument() {
            assert_eq!(
                ...
                "#[nexum_sdk::module] takes no arguments except `sol_events(EventType, ...)`",
            );
        }
        ...
        fn trailing_tokens() {
            assert_eq!(
                ...
                "unexpected tokens after `sol_events(...)`",
            );
        }
        ...
        fn empty_sol_events() {
            assert_eq!(
                MacroError::EmptySolEvents { span: site() }.to_string(),
                "`sol_events(...)` must name at least one event type",
            );
        }
        ...
        fn unknown_handler() {
            assert_eq!(
                ...
                "`on_blocks` is not a recognized #[nexum_sdk::module] handler; expected one of \
                 [\"init\", \"on_block\", \"on_event\", \"on_schedule\", \"on_extension\"] \
                 (rename helpers so they do not start with `on_`)",
            );
        }
        ...
        fn no_handlers() {
            assert_eq!(
                ...
                "#[nexum_sdk::module] found no recognized handlers on this impl; define at least \
                 one of `init`, `on_block`, `on_event`, `on_schedule`, `on_extension`",
            );
        }
        ...
        #[test]
        fn sol_events_without_event_trigger() {
            let err = MacroError::SolEventsWithoutEventTrigger {
                path: "/m/component.toml".into(),
            };
            assert_eq!(
                err.to_string(),
                "`sol_events(...)` names events, but /m/component.toml declares no event \
                 trigger with an `event_signature`; add the trigger or drop the argument",
            );
        }
    }
}
