...
//! - [`Poller`] - world-neutral poll seam: one commitment in, one
//!   outcome out at a [`Tick`].
//! - [`Retrier`] - runs a [`RetryAction`] through the stores after a
//!   failed run.
//!
//! [`CommitmentRef`] derives gate and marker keys from the verbatim hex
//! substrings of the stored commitment key; [`CommitmentSet::remove`]
//! drops a commitment with all its derived keys so no failure path
//! orphans one.
//!
//! ```
//! use nexum_sdk::keeper::{CommitmentRef, CommitmentSet, Gates, Journal};
...
//! let key = commitments.put(&Address::ZERO, &B256::ZERO, b"params")?;
//! let commitment = CommitmentRef::parse(&key).expect("well-formed key");
//!
//! let gates = Gates::new(&host);
//! gates.set_next_block(commitment, 100)?;
//! assert!(!gates.is_ready(commitment, 99, 0)?);
//! assert!(gates.is_ready(commitment, 100, 0)?);
//!
...
//! assert!(commitments.list()?.is_empty());
//! # Ok::<(), Fault>(())
//! ```
...
/// First-refusal marker paired with a commitment: block of the first
/// [`RetryAction::DropOnRepeat`] refusal, u64 little-endian.
pub const REFUSED_PREFIX: &str = "refused:";

/// Canonical commitment key for an owner / commitment-hash pair;
/// lowercase `0x`-prefixed hex on both halves.
#[must_use]
pub fn commitment_key(owner: &Address, hash: &B256) -> String {
    format!("{COMMITMENT_PREFIX}{owner:#x}:{hash:#x}")
}

/// Borrowed view of a commitment key's two hex halves. Derived keys
/// reuse the verbatim substrings, so parse-then-derive is byte-stable
/// regardless of the original hex casing.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct CommitmentRef<'k> {
    owner_hex: &'k str,
    hash_hex: &'k str,
}
...
impl<'k> CommitmentRef<'k> {
    ...
    /// Rebuild the full commitment key.
    pub fn key(&self) -> String {
        format!("{COMMITMENT_PREFIX}{}:{}", self.owner_hex, self.hash_hex)
    }

    /// The `next_block:` gate key paired with this commitment.
    pub fn next_block_key(&self) -> String {
        format!("{NEXT_BLOCK_PREFIX}{}:{}", self.owner_hex, self.hash_hex)
    }

    /// The `next_epoch:` gate key paired with this commitment.
    pub fn next_epoch_key(&self) -> String {
        format!("{NEXT_EPOCH_PREFIX}{}:{}", self.owner_hex, self.hash_hex)
    }

    /// The `refused:` first-refusal marker key paired with this
    /// commitment.
...
impl<'h, H: LocalStoreHost> CommitmentSet<'h, H> {
    ...
    pub fn key(owner: &Address, hash: &B256) -> String {
        commitment_key(owner, hash)
    }

    /// Insert or overwrite the commitment row; returns the key written.
    ...
    /// The stored value. `Ok(None)` when the commitment is absent.
    pub fn get(&self, commitment: CommitmentRef<'_>) -> Result<Option<Vec<u8>>, Fault> {
        self.host.get(&commitment.key())
    }

    /// Every commitment key currently registered.
    pub fn list(&self) -> Result<Vec<String>, Fault> {
        self.host.list_keys(COMMITMENT_PREFIX)
    }

    /// Drop the commitment with its gate and refusal keys. Derived keys
    /// go first, so a fault leaves the commitment row for a retry and
    /// never orphans a derived key.
    pub fn remove(&self, commitment: CommitmentRef<'_>) -> Result<(), Fault> {
        Gates::new(self.host).clear(commitment)?;
        self.host.delete(&commitment.refused_key())?;
        self.host.delete(&commitment.key())
    }
}
...
/// corrupt value can only make a commitment poll sooner, never wedge
/// it).
...
impl<'h, H: LocalStoreHost> Gates<'h, H> {
    ...
    pub fn set_next_block(&self, commitment: CommitmentRef<'_>, block: u64) -> Result<(), Fault> {
        self.host
            .set(&commitment.next_block_key(), &block.to_le_bytes())
    }

    /// Skip polls until the Unix-seconds clock reaches `epoch_s`.
    pub fn set_next_epoch(&self, commitment: CommitmentRef<'_>, epoch_s: u64) -> Result<(), Fault> {
        self.host
            .set(&commitment.next_epoch_key(), &epoch_s.to_le_bytes())
    }

    /// Whether the commitment is clear to poll. Both gates must pass;
    /// each is inclusive at its threshold.
    ...
    pub fn is_ready(
        ...
        if let Some(next) = read_u64(self.host, &commitment.next_block_key())?
            && block < next
        {
            return Ok(false);
        }
        if let Some(next) = read_u64(self.host, &commitment.next_epoch_key())?
            && epoch_s < next
        {
            return Ok(false);
        }
    ...
    pub fn clear(&self, commitment: CommitmentRef<'_>) -> Result<(), Fault> {
        self.host.delete(&commitment.next_block_key())?;
        self.host.delete(&commitment.next_epoch_key())
    }
}
...
/// Gate checks and backoff read the instant the upstream was polled at,
/// so a commitment never gates against a clock it was not judged by.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Tick {
...
/// A probe of the upstream: poll one commitment, produce one outcome.
/// Generic over the host for mock-testability; the probe owns its own
/// wire. `poll` is infallible by contract: a transient failure surfaces
/// as a retry-flavoured outcome rather than tearing down the run.
pub trait Poller<H> {
    /// What one poll produces.
    type Outcome;

    /// Probe the upstream for `commitment` at `tick`. `params` is the
    /// stored commitment value, passed verbatim for the probe to
    /// decode.
    fn poll(
        &self,
        host: &H,
...
/// What the retry ledger does to a commitment after a failed run.
/// `#[non_exhaustive]`: treat an unknown variant as leave-in-place.
#[derive(Clone, Copy, Debug, Eq, PartialEq, IntoStaticStr)]
#[strum(serialize_all = "snake_case")]
#[non_exhaustive]
pub enum RetryAction {
    /// Leave the commitment untouched; the next tick re-attempts.
    TryNextBlock,
    /// Gate the commitment until `now + seconds` on the epoch clock.
    Backoff {
    ...
    /// the commitment.
    DropOnRepeat,
    /// Remove the commitment and its gates; no retry can succeed.
    Drop,
}
...
/// a later block removes the commitment; `Drop` delegates to
/// [`CommitmentSet::remove`], so derived keys go first and no failure
/// path can orphan one.
...
impl<'h, H: LocalStoreHost> Retrier<'h, H> {
    ...
    /// Apply `action` to the commitment at `tick`: the backoff origin
    /// and the repeat-detection block both read from it.
    pub fn apply(
        &self,
        commitment: CommitmentRef<'_>,
        action: RetryAction,
        ...
        match action {
            ...
            RetryAction::Backoff { seconds } => Gates::new(self.host)
                .set_next_epoch(commitment, tick.epoch_s.saturating_add(seconds)),
            RetryAction::DropOnRepeat => {
                let key = commitment.refused_key();
                match read_u64(self.host, &key)? {
                    Some(block) if tick.block > block => {
                        CommitmentSet::new(self.host).remove(commitment)
                    }
                    Some(_) => Ok(()),
                    None => {
                        self.host.set(&key, &tick.block.to_le_bytes())?;
                        Gates::new(self.host)
                            .set_next_block(commitment, tick.block.saturating_add(1))
                    }
                }
            }
            RetryAction::Drop => CommitmentSet::new(self.host).remove(commitment),
        }
    }
    ...
    /// No-op when unset.
    pub fn clear_refusal(&self, commitment: CommitmentRef<'_>) -> Result<(), Fault> {
        self.host.delete(&commitment.refused_key())
    }
}
