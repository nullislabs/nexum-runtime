...
Against a compromised artifact store, supply an operator-owned manifest from outside the artifact directory via the `manifest` key on `[[modules]]`/`[[services]]`, combined with `require_component_digest = true`.
