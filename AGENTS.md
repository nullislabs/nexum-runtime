...
- `crates/nexum-tasks` - task lifecycle and graceful shutdown, and the only crate that spawns raw `tokio` tasks. The root `clippy.toml` backs this rule: it disallows the `tokio` free spawn functions workspace-wide.
